  ...
  .then((map) => {
    ...
    class Room {
      constructor(room_id, title, coordinates, items, exits, cooldown) {
        ...
        this.title = title;
        this.coordinates = [Number(coordinates.replace(/[(](\d+)\s?[,]\s?(\d+)[)]/, '$1')), Number(coordinates.replace(/[(](\d+)\s?[,]\s?(\d+)[)]/, '$2'))];
        this.items = items;
        this.exits = exits;
    ...
    */
        ...
        while (Object.keys(treasureMap).length < 2) {
          ...
          console.log(`treasureMap ${((Object.keys(treasureMap).length)/500) * 100}%  ${Object.keys(treasureMap).length}`)
          ...
          let prevDirection = [...traversalPath].pop()
            ...
            if (nextDirection === null) {
              ...
              if (!(Object.values(treasureMap[current_room.room_id]).includes('?'))) {
                  completeMap.filter(place => place.room_id)[0].exits = treasureMap[current_room.room_id]
              }
            }
